 ...
 * 规范化后的配置
 */
export interface NormalizedConfig extends Required<Omit<AiAssistantConfig, 'onMessage' | 'onToken' | 'onToolCall' | 'onError' | 'onFinish' | 'onReconnect' | 'onDisconnect'>> {
  mcp?: string; // MCP 服务器 ID（可选）
  onMessage?: (message: Message) => void;
  onToken?: (token: string) => void;
  onToolCall?: (toolName: string, args: any) => void;
  onError?: (error: Error) => void;
